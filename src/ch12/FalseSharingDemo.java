...
public class FalseSharingDemo extends Thread {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int N;// 工作者线程数
        N = argc > 0 ? Integer.parseInt(args[0]) : Runtime.getRuntime()
                .availableProcessors();
        long iterations;
        iterations = argc > 1 ? Long.parseLong(args[1]) : 400 * 1000 * 1000L;

        String taskImplClassName;
        taskImplClassName = System.getProperty("x.task.impl");
        if (taskImplClassName == null) {
            taskImplClassName = "ManuallyPaddedCountingTask";
        }
    ...
    private static CountingTask[] createTasks(String taskImplClassName, int N, long iterations) {
